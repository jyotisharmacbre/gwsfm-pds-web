...
export const calculateCost = (noOfHours: number, hourRate: number) => {
	let totalCost = 0;
	if (noOfHours > 0 && hourRate > 0) totalCost = noOfHours * hourRate;
...
export const calculateTotalCost = (cost: number) => {
	...
	adminDefaultData.map((x) => {
		if (x.name == AdminFields.InsuranceRatePerc) {
			insurranceCost = parseFloat(x.value);
		}
	});
...
export const calculateClientDiscount = (discountType: number, cost: number, value: number) => {
	...
	else if (discountType == 2) return value;
	else return ((cost > 0 ? cost : 0) * (value / 100)).toFixed(2);
};
