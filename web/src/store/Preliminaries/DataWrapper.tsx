...
import { IAdminDefaults } from '../Admin/Types/IAdminDefault';
...
export const bindUserData = (preliminaryData) => {
	...
	let defaultGrossMargin: number = 0;
	let labourRate: number = 0;
	adminDefaultData.map((x) => {
		if (x.name == AdminFields.GrossMarginPerc) {
			defaultGrossMargin = parseInt(x.value);
		}
		if (x.name == AdminFields.CBRELabourRatePerc) {
			labourRate = parseInt(x.value);
		}
	});
	if (lookupData && lookupData.length > 0) {
		var pre_components = lookupData.filter((data) => {
			return data.lookupItem == LookupField.Pre_Components;
		});
		var pre_component_items = lookupData.filter((data) => {
		...
		pre_components.map((components) => {
			...
			var preData =
				preliminaryData != ''
					? preliminaryData.filter((x) => {
							return x.preliminariesComponentId == components.lookupKey;
						})
					: [];
...
const generateItemDetails = (preData, item, labourRate, defaultGrossMargin) => {
	...
	let itemData = preData.filter((itemData) => {
		return itemData.preliminariesItemId == item.lookupKey.toString();
	});
	...
	items.nameOfSupplier =
		itemData.length > 0 ? (itemData[0].nameOfSupplier == null ? '' : itemData[0].nameOfSupplier) : '';
	items.noOfHours = itemData.length > 0 ? itemData[0].noOfHours : 0;
	items.hourRate =
		itemData.length > 0 ? itemData[0].hourRate : CheckConstraints(items.itemName.toUpperCase()) ? labourRate : 0;
...
export const convertIntoDatabaseModel = (userData, projectId) => {
	...
	userData.map((components) => {
		components.items.map((items) => {
			let preliminaries: IPreliminaries = {
				PreliminaryId: '',
				ProjectId: '',
				PreliminariesItemId: '',
				PreliminariesComponentId: '',
				NameOfSupplier: '',
				NoOfHours: 0,
				HourRate: 0,
				TotalCost: 0,
