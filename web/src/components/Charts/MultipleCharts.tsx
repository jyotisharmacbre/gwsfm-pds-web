...
Chart.helpers.extend(Chart.controllers.doughnut.prototype, {
  draw: function() {
    ...
    if (chart.config.options.showfirstItemLabel) {
    ...
    if (chart.config.options.showTotalLabel) {
      var text2 = sum,
        textX1 = Math.round((width - ctx.measureText(text2).width) / 2),
        textY1 = height / 2;
      ctx.fillText(text2, textX1, textY1);
    }
  }
});
...
export default function MultipleChart(props: {
  ProjectTotal: number;
  ICE: number;
  Rejected: number;
  JandA: number;
  ...
  InProgress: number;
  Completed: number;
  OnHoldProject: number;
}) {
  return (
    <Grid container spacing={0} min-Height={350} id="GridMultipleChart">
      <Grid
        item
        xs={6}
        sm={6}
